export const get3DAvatarUrl = (name: string = "User") => {
  const palettes = [
  ...
  <defs>
    <linearGradient id="hg-${uniqueId}" x1="0%" y1="0%" x2="100%" y2="100%">
    ...
    <radialGradient id="hh-${uniqueId}" cx="30%" cy="30%" r="50%">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.8"/>
      <stop offset="100%" stop-color="#ffffff" stop-opacity="0"/>
    </radialGradient>
    <radialGradient id="bh-${uniqueId}" cx="30%" cy="10%" r="60%">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.6"/>
      <stop offset="100%" stop-color="#ffffff" stop-opacity="0"/>
    </radialGradient>
    <filter id="ds-${uniqueId}" x="-20%" y="-20%" width="140%" height="140%">
      <feDropShadow dx="0" dy="4" stdDeviation="3" flood-color="${p.shadow}" flood-opacity="0.3"/>
    </filter>
  </defs>
  <g filter="url(#ds-${uniqueId})" transform="translate(0, 5)">
    <path d="M 15 95 C 15 60 25 50 50 50 C 75 50 85 60 85 95" fill="url(#bg-${uniqueId})" stroke="${p.stroke}" stroke-width="2" />
    <path d="M 15 95 C 15 60 25 50 50 50 C 75 50 85 60 85 95" fill="url(#bh-${uniqueId})" />
    <circle cx="50" cy="32" r="24" fill="url(#hg-${uniqueId})" stroke="${p.stroke}" stroke-width="2" />
